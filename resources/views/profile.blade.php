...
<main class="p-10">
    ...
    @endif
    <div class="mb-12">
        <h1 class="text-4xl text-[#C4FFCE] font-bold">Manage Your Profile</h1>
    ...
    <div class="grid grid-cols-3 gap-8 mb-12">
        <div class="bg-[#8DB295] p-6 rounded-2xl shadow-md">
            <p class="text-lg text-[#040604]">Your username: <span class="font-bold">{{ $user->username }}</span></p>
            <div x-data="{ modalOpen: false }"
                 @keydown.escape.window="modalOpen = false"
                 class="relative z-50 w-auto h-auto">
                <button @click="modalOpen=true" class="flex-1 mt-6 bg-[#040604] text-[#C4FFCE] rounded-lg px-4 py-2">✎ Edit Username</button>
                    ...
                    <div x-show="modalOpen" class="fixed top-0 left-0 z-[99] flex items-center justify-center w-screen h-screen" x-cloak>
                        ...
                        <div x-show="modalOpen"
                            ...
                            <div class="flex items-center justify-between pb-2">
                                <h3 class="text-lg font-semibold">Edit Username</h3>
                                <button @click="modalOpen=false" class="absolute top-0 right-0 flex items-center justify-center w-8 h-8 mt-5 mr-5 text-gray-600 rounded-full hover:text-gray-800 hover:bg-gray-50">
                                    <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                                </button>
                            </div>
                            <div class="relative w-auto">
                                <form action="{{ route('profile.editUsername') }}" method="POST" class="flex flex-col gap-2">
                                    @csrf
                                    <label for="newusername">Enter your desired username</label>
                                    <input type="text" name="username" class="p-2 rounded-md bg-[#C4FFCE] text-[#040604] placeholder-[#040604]/70" placeholder="Enter your new username...">
                                    <button type="submit" class="p-2 rounded-md bg-[#C4FFCE] mt-2 text-[#040604] hover:border hover:bg-[#040604] hover:border-[#C4FFCE] hover:text-[#C4FFCE] duration-300 font-bold">Edit Username</button>
                                </form>
                            </div>
                        </div>
                    </div>
        ...
        <div class="bg-[#8DB295] p-6 rounded-2xl shadow-md">
            <p class="text-lg text-[#040604]">Edit Password</p>
            <div x-data="{ modalOpen: false }"
                 @keydown.escape.window="modalOpen = false"
                 class="relative z-50 w-auto h-auto">
                <button @click="modalOpen=true" class="flex-1 mt-6 bg-[#040604] text-[#C4FFCE] rounded-lg px-4 py-2">✎ Edit Password</button>
                <template x-teleport="body">
                    <div x-show="modalOpen" class="fixed top-0 left-0 z-[99] flex items-center justify-center w-screen h-screen" x-cloak>
                        <div x-show="modalOpen"
                             x-transition:enter="ease-out duration-300"
                        ...
                        <div x-show="modalOpen"
                            ...
                            <div class="relative w-auto">
                                <form action="{{ route('profile.editPassword') }}" method="POST" class="flex flex-col gap-2">
                                    @csrf
        ...
        <div class="bg-[#8DB295] p-6 rounded-2xl shadow-md">
            ...
            @foreach($wallets as $wallet)
                <h2 class="text-[#040604] text-md font-black mt-2">{{ $wallet->address }} / {{ $wallet->type }}</h2>
            @endforeach
        </div>
    </div>
